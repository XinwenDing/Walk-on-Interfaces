function [step_h_ux, step_h_grad_ux, y, Q] = woi_uniform(x, b, D, hierarchy_matrix, Phi, grad_Phi, NP_Operator_kernel, exp_weight, ...
    ...
    if depart_schedule == -1 % first step leaving from x
        for i = 1 : N
            ...
            bi = b{i};
            p0 = uniform_pdf(i);
            [yi, n_yi] = D.uniform_ellipsoid_sampler(omega_i, dim, total_length_i, cowalker_num);
        ...
        for i = 1 : N
            ...
            [yi_ancestor, ~] = get_ellipsoid_intersection(prev_yi(walk_to_ancestor_idx,:), omega_i, cowalker_num);
            [yi_self, ~] = get_ellipsoid_intersection(prev_yi(walk_to_self_idx,:), omega_i, cowalker_num);
            [yi_descendent, n_yi_descendent] = D.uniform_ellipsoid_sampler(omega_i, dim, descendent_num, cowalker_num);
