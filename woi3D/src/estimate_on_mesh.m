function [ux, grad_ux] = estimate_on_mesh(x, b, D, hierarchy_matrix, interface_weight, Markov_tree_height, schedule_per_batch, batch_num)
    ...
    for batch_idx = 1 : batch_num
        [batch_interface_schedule, batch_sign_schedule, schedule_per_batch] = schedule_target_interface(interface_weight, Markov_tree_height, N, schedule_per_batch);
        step_ux = zeros(1, 1, query_num, 'gpuArray'); step_grad_ux = zeros(1, dim, query_num, 'gpuArray');
        depart_schedule = -1; prev_y = -1; prev_y_tri_idx = -1; prev_ny = -1;
        ...
        for h = 1 : Markov_tree_height
            ...
            [step_h_ux, step_h_grad_ux, y, y_tri_idx, ny, Q, T] = woi_on_mesh(x, b, D, hierarchy_matrix, Phi, grad_Phi, NP_Operator_kernel, exp_weight, ...
